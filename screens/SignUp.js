...
const SignUpScreen = ({navigation}) => {
  ...
  navigation.setOptions({
    ...
    headerShadowVisible: false, // Ensure the header shadow is hidden
  })
})
  ...
  const openCamera = async () => {
    ...
    if (cameraPermission === false) {
      alert("Sorry camera access has not been granted by the user. This can be changed in the app settings of your phone");
      return;
    }
  ...
  const pickImage = async () => {
    ...
    {
      alert("Sorry image storage access has not been granted by the user. This can be changed in the app settings of your phone");
      return;
    }
  ...
  const createUserRef = async () =>{
    ...
    catch(err){console.log("Something went wrong with storing the user: ",err)}
  }
  ...
  async function signUp() {
    if (value.email === '' || value.password === '' || value.firstname === '') {
      setValue({
        ...value,
        error: 'Email and Password and First Name are required.'
      })
      return;
    }
    try{
      ...
      .then(async ()=>{
        await auth.currentUser.updateProfile({
          ...
          photoURL: value.file || 'https://www.pngall.com/wp-content/uploads/12/Avatar-PNG-Images-HD.png'
        })
    ...
    <ImageBackground source={require('../assets/signup.jpg')} resizeMode="cover" style={styles.image}>
      <ScrollView style={styles.scrollContainer}>
        <View style={styles.greetingContainer}>
          ...
          <Text style={styles.profileImageText}>Click the image to upload your profile pic</Text>
        </View>
          ...
          <View style={styles.profileImageContainer}>
            <View style={styles.profileImageButton}>
              <TouchableOpacity onPress={pickImage} activeOpacity={0.5} >
                <View >
                  <Image style={styles.profileImage} source={{uri: value.file || 'https://www.pngall.com/wp-content/uploads/12/Avatar-PNG-Images-HD.png'}}/>
                </View>
              </TouchableOpacity>
            </View>
