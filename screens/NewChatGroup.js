...
import { StyleSheet, Text, View, TextInput, Button, KeyboardAvoidingView, Platform, ScrollView, Dimensions } from 'react-native';
...
const NewChatGroupScreen = ({navigation}) => {
  ...
  const getAllUser = async () => {
    ...
    try{
      ...
      .catch(err=>console.log('Problem getting all users => ',err))
      .then(()=>{
        setAllUser(temp);
    ...
    try{
      const docRef = await database.collection("chats").doc(chatID(chateeID)).set({
        ...
        const docRef2 = await database.collection('chats').doc(chatID(chateeID)).collection("messages").add({
          timeStamp: firebase.firestore.FieldValue.serverTimestamp(),
          message: `Welcome users to the private chat created on ${time}`,
          user: { _id: "N/A",
                  name: "ChatApp",
                  email: "N/A",
                  photoURL: 'https://www.pngall.com/wp-content/uploads/12/Avatar-PNG-Images-HD.png'}
        })
      }).then(async () => {
    ...
    try{
      ...
      .then(async (value) =>{
        const docRef2 = await database.collection('chats').doc(value.id).collection("messages").add({
          timeStamp: firebase.firestore.FieldValue.serverTimestamp(),
          message: `Welcome users to the Group Chat "${chatGroup}" created on ${time}`,
          user: { _id: "N/A",
                  name: "ChatApp",
                  email: "N/A",
                  photoURL: 'https://www.pngall.com/wp-content/uploads/12/Avatar-PNG-Images-HD.png'}
      ...
      .catch(console.log("Message is being added, this may take a moment"))
      navigation.goBack()
    } catch(err){
      console.error("Error adding document: ", err)
    }
